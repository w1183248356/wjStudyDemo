...
import com.wjstudydemo.BuildConfig;
...
public class AppForegroundStateManager {
    ...
    private void notifyListeners(AppForegroundState newState) {
        if (BuildConfig.DEBUG)
            Log.i("ii", "Notifying subscribers that app just entered state: " + newState);
    ...
    private void validateThenNotifyListeners() {
        ...
        if (mHandler.hasMessages(MESSAGE_NOTIFY_LISTENERS)) {
            if (BuildConfig.DEBUG)
                Log.i("ii", "Validation Failed: Throwing out app foreground state change notification");
        ...
        public void handleMessage(Message inputMessage) {
            switch (inputMessage.what) {
                ...
                case MESSAGE_NOTIFY_LISTENERS:
                    /* 通知所有观察者状态发生了改变 */
                    if (BuildConfig.DEBUG)
                        Log.i("ii", "App just changed foreground state to: " + mAppForegroundState);
