 ...
 * @author clifford
 */
public class MyCopyImageView extends ImageView {
     ...
     * @see #getPinchMode()
     */
    public static final int PINCH_MODE_SKEW = 3;
    ...
    public boolean canScrollHorizontally(int direction) {
        if (mPinchMode == MyCopyImageView.PINCH_MODE_SCALE) {
            return true;
        }
    ...
    public interface OuterMatrixChangedListener {
         ...
         */
        void onOuterMatrixChanged(MyCopyImageView pinchImageView);
    }
    ...
    public MyCopyImageView(Context context) {
        super(context);
        initView();
    }

    public MyCopyImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public MyCopyImageView(Context context, AttributeSet attrs, int defStyle) {
     ...
     * 在onTouchEvent末尾被执行.
     */
    private GestureDetector mGestureDetector = new GestureDetector(MyCopyImageView.this.getContext(), new GestureDetector.SimpleOnGestureListener() {

        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            //只有在单指模式结束之后 并且进行了缩放处理才允许执行fling
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (mOnLongClickListener != null) {
                mOnLongClickListener.onLongClick(MyCopyImageView.this);
            }
        }
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            if (mOnClickListener != null) {
                mOnClickListener.onClick(MyCopyImageView.this);
            }
            return true;
        }
    });
...
//                    skew();
            ...
            case MotionEvent.ACTION_MOVE://当有点在屏幕上移动时触发
                if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                    if (mPinchMode == PINCH_MODE_SCALE && event.getPointerCount() > 1) {
                        //获得两指之间长度
                        ...
                        //滚动模式下滚动
                    } else if (mPinchMode == PINCH_MODE_SCROLL) {
                        scrollBy(event.getX() - mLastMovePoint.x, event.getY() - mLastMovePoint.y);
                        mLastMovePoint.set(event.getX(), event.getY());
                    } else if (mPinchMode == PINCH_MODE_SKEW && event.getPointerCount() > 1) {
                        //获得两指中点坐标
//                        float[] lineCenter = MathUtils.getCenterPoint(event.getX(0), event.getY(0), event.getX(1), event.getY(1));
                        //保存两指中点坐标
//                        mLastMovePoint.set(lineCenter[0], lineCenter[1]);
//                        skew();
    ...
    private void skew() {
        if (!isReady()) return;
        Matrix matrix = MathUtils.matrixTake();
        matrix.postSkew(1f, 0f, getWidth() / 2, getHeight() / 2);
    ...
    private boolean scrollBy(float xDiff, float yDiff) {
        ...
        RectF bount = MathUtils.rectFTake();
        getImageBound(bount);
        float disWidth = getWidth();
        float disHeight = getHeight();
        //如果图片的宽度 < 宽度 不能移动
        if (bount.right - bount.left < disWidth) {
            xDiff = 0;
            //如果图片左坐标 + 移动距离  > 0
        } else if (bount.left + xDiff > 0) {
            if (bount.left < 0) {//图片开始小于0
//                xDiff = xDiff - (bount.left + xDiff);
                xDiff = -bount.left;
            } else {
                xDiff = 0;
            }
            //如果移动之后 小于 控件右坐标
        } else if (bount.right + xDiff < disWidth) {
            //移动之前没有超出
            if (bount.right > disWidth) {
                xDiff = disWidth - bount.right;
            } else {
                xDiff = 0;
            }
        }
        ...
        //如果图片的长度 < 长度 不能移动
        if (bount.bottom - bount.top < disHeight) {
            yDiff = 0;
            //如果图片左坐标 + 移动距离  > 0
        } else if (bount.top + yDiff > 0) {
            if (bount.top < 0) {//图片开始小于0
//                xDiff = xDiff - (bount.left + xDiff);
                yDiff = -bount.top;
            } else {
                yDiff = 0;
            ...
            //如果移动之后 小于 控件右坐标
        } else if (bount.bottom + yDiff < disHeight) {
            //移动之前没有超出
            if (bount.bottom > disHeight) {
                yDiff = disHeight - bount.bottom;
            } else {
                yDiff = 0;
            }
        }

        MathUtils.rectFGiven(bount);
    ...
    private void scale(PointF scaleCenter, float scaleBase, float distance, PointF lineCenter) {
        ...
        //MSCALE_X *  新两手指距离/ 旧两手指距离
        //MSCALE_X（整体缩放比例） *  最新的缩放比例
...
//        matrix.postRotate(i, getWidth() / 2, getHeight() / 2);
    ...
    private void scaleEnd() {
        ...
        if (currentScale > maxScale) {
            scalePost = maxScale / currentScale;//为了缩放
        }
    ...
    public static class MathUtils {
        ...
        public static float[] getMatrixScale(Matrix matrix) {
            if (matrix != null) {
                ...
                return new float[]{value[0], value[4]};//获得MSCALE_X， MSCALE_Y
            } else {
                return new float[2];
            }
        }
